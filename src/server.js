...
// ── FALLBACK RESPONSE ────────────────────────────────────
const FALLBACK_RESPONSE = `Hi, I am ${BOT_NAME}! I am just finishing my startup — please try again in about a minute and I will be ready to answer any questions about VDart.`;
...
async function getWebsiteContent() {
  if (!cachedContent || Date.now() - lastFetched > ONE_HOUR) {
    if (!scrapeInProgress) {
      ...
      console.log("Scraping website in the background...");
      Promise.all(
...
2. If the answer is not in the website content, you may search for topics directly related to VDart, VDart Digital, VDart Academy, or Sidd Ahmed.
...
- Greet users by saying: "Hi, I'm ${BOT_NAME}! How can I help you with VDart today?"
...
- Respond: "Hi, I am ${BOT_NAME}! How can I help you with VDart today?"
...
app.post("/chat", apiLimiter, preventParallelRequests, async (req, res) => {
  try {
    ...
    // Optimize website content (naive truncation for now to save tokens)
    const optimizedContent = websiteContent.length > 5000 ? websiteContent.substring(0, 5000) + '... (truncated)' : websiteContent;
...
app.listen(PORT, () => {
  ...
  // Start scraping on boot
  getWebsiteContent().catch(err => console.error(err));
});